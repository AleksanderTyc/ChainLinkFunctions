...
contract CLFunctionTemperatureInsurer is FunctionsClient, ConfirmedOwner {
  ...
  event Response(bytes32 indexed requestId, uint256 temperature, bytes response, bytes err);
...
"}";
   ...
   */
  constructor(uint256 _adverseTemperature) FunctionsClient(router) ConfirmedOwner(msg.sender) {
  ...
  function fulfillRequest(
    ...
    if( (temperature < adverseTemperature) && (err.length == 0) ) {
    claimStatus = 2;
    ...
    function setClaim(uint256 _claimStatus) onlyOwner public {
