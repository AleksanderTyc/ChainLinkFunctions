...
import { ContractPurchase } from './ContractPurchase.jsx';
import { UpdateTemperature } from './UpdateTemperature.jsx';
import { UpdateStatus } from './UpdateStatus.jsx';
import { UpdateAdverseTemperature } from './UpdateAdverseTemperature.jsx';
...
import {
    ...
    cResetContract,
    checkTxIndex
} from './TemperatureInsurer_ABI.js';
...
// Sepolia
const TemperatureInsurer_Interface = { addressContract: "0x1b1397564ef39C24c213A0a4Cf104fBA37fe7698" };
    ...
    return (
        <div id="hero">
            ...
            <ContractInfo
                ...
                pOwner={cOwner}
                pStatus={cStatus}
                pAdverseTemperature={cAdverseTemperature}
                pInsured={cInsured}
                pPremium={cPremium}
                pLatitude={cLatitude}
                pLongitude={cLongitude}
                pCurrTemperature={cCurrTemperature}
                pClaimStatus={cClaimStatus}
            />
            {appStatus === "Owner" && <UpdateAdverseTemperature
                handler={updAdvTemp}
                defaultTemp={-2.5}
            />}
            {appStatus === "Owner" && <UpdateStatus
                handler={updStatus}
                defaultStatus={2}
            />}
            {appStatus === "Owner" && <UpdateTemperature
                handler={updStatusWithTemp}
                defaultTemp={9.5}
            />}
            {appStatus !== "Owner" && cStatus === "Contract Available" && <ContractPurchase
                handler={submitBuy}
                defaultLat={53.1222619761935}
                defaultLong={17.9989758234834}
            />}
            <section>
                <div id="hbar"></div>
                <div id="passwds-cont">
                    {appStatus === "Owner" && <button id="inc-btn" onClick={submitReset}>Contract Reset</button>}
                    {appStatus !== "Owner" && cStatus === "Claim Available" && <button id="upd-btn" onClick={submitClaim}>Claim</button>}
                </div>
            </section>
        </div>
    );
